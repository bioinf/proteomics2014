 ...
 * Date: 19.03.14
 */
public class WPGMA implements IHierarchicalClustering {
    ...
    public void build(ISequence[] seqArr, int[][] pairwiseAlignmentMatrix) {
        ...
        // convert each sequence to a single cluster
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                matrix[i][j] = pairwiseAlignmentMatrix[i][j];
            }
        }
    ...
    protected void buildGuideTree() {
        ...
        while (nodeList.size() > 1) {
            ...
            // find max score
            PairInd max = findMaxScore(n);

            // join clusters
            GuideTreeNode joinNode = new GuideTreeNode(nodeList.get(max.i), nodeList.get(max.j), matrix[max.i][max.j]);
            if (Config.DEBUG) {
                joinNode.seq = new Sequence(joinNode.left.seq.getDescription() + joinNode.right.seq.getDescription(), "");
            }

            // calc scores for the cluster
            double[] newScores = recalcScore(max.i, max.j, n);

            // remove old nodes and insert the new one
            removeOldInsertNew(max.i, max.j, joinNode, n, newScores);
        }
    }
    ...
    protected double[] recalcScore(int i, int j, int n) {
        ...
        for (int k = 0; k < n; k++) {
            ...
            newScore[k] = (matrix[i][k] + matrix[j][k]) / 2;
        }
        return newScore;
    }
    ...
    protected PairInd findMaxScore(int n) {
        int maxI = -1;
        int maxJ = -1;
