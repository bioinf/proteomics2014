...
public class Fasta {
    ...
    /**
     * Read sequences from the input stream and write them to the array list
     * @param in input stream with sequences in FASTA format
     * @param sequenceFactory common factory
     */
    public static ArrayList<? extends ISequence> readSequences(In in, ISequenceFactory sequenceFactory) {
    ...
    public static ArrayList<? extends ISequence> readSequences(String filename, ISequenceFactory sequenceFactory) {
        ...
        ArrayList<? extends ISequence> sequences = readSequences(in, sequenceFactory);
        in.close();
        return sequences;
    }

    public static Map<String, ? extends ISequence> readMap(In in, ISequenceFactory sequenceFactory) {
        ArrayList<? extends ISequence> arrayList = readSequences(in, sequenceFactory);
    ...
    public static Map<String, ? extends ISequence> readMap(String filename, ISequenceFactory sequenceFactory) {
        In in = new In(filename);
        Map<String, ? extends ISequence> map = readMap(in, sequenceFactory);
        in.close();
        return map;
    }
